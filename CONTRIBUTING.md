...
**리뷰어는 판정 축이 아니라 컨텍스트 조건으로 나뉜다.**  
같은 입력으로 판정되는 축은 한 리뷰어가 패스로 순차 처리한다.  
축마다 리뷰어를 띄우면 같은 문서를 같은 조건에서 읽은 판정이 여러 벌 나오고 그 판정들은 서로 독립이 아니다.

「발동」의 필수는 항상 띄운다는 뜻이고 조건부는 리뷰 스킬이 정한 조건을 만족할 때만 띄운다는 뜻이다.  
그 조건은 `skills/design-review/SKILL.md`의 「Reviewer roster」에 있다.

| 리뷰어     | 발동   | 무엇을 갖고 보나                 | 도는 축                                                                    |
|------------|--------|----------------------------------|----------------------------------------------------------------------------|
| `unaided`  | 필수   | 문서 경로만                      | 판독 여덟 종 · `모호` · `모순` · `결손` · `기준` · `교차` · `무근거` · `대안` · `과폭` |
| `origin`   | 필수   | 문서 + 사용자 발화 원문 verbatim | `의도` · `누락요구` · `단정` · `미확정` · `초과`                           |
| `external` | 조건부 | 문서 + 조회 도구                 | `사실` · `미확인` · `연관`                                                 |
| `audience` | 조건부 | 문서 + 사용자 대면 요소          | `첫사용` · `반복` · `문구` · `부담전가`                                    |

`origin`은 필수지만 사용자 발화 원문이 세션 휘발로 없으면 뺀다.  
뺐다는 사실을 사용자에게 밝힌다.

리뷰어 전원이 쓰는 반환 계약은 `skills/design-review/reviewer-contract.md` 한 곳에 있다.  
리뷰어가 출력 직전에 스스로 Read하므로 프롬프트에 주입하지 않는다.

`unaided`의 판독 축만 예외로 정의가 `skills/writing-for-junior/junior-read-prompt.md`에 있다.  
판정 기준과 작성 규범이 같은 지식의 양면이라 `writing-for-junior`가 함께 소유한다.  
그 파일도 주입하지 않고 `unaided`가 판독 패스를 시작할 때 Read한다.
