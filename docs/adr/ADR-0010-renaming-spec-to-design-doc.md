# ADR-0010: 산출물을 스펙에서 설계 문서로 개명한다
...
ADR-0008이 이 산출물에 「전역 제약」과 「수용 기준」을 더해 계약으로 만들었고, 그 결과 문서의 내용이 이름과 더 멀어졌다.
...
이 가운데 「설계」의 정의가 "무엇을 어떻게 만드는가"이고, 고른 것 옆에 버린 대안과 버린 이유를 함께 적게 한다.
...
**자기 규정을 하나로 모은다.**
`기획 문서`는 버린다.
기획은 요구 쪽을 가리키는 말이라 「설계」 절과 정면으로 어긋난다.
남기는 규정은 둘이고 층위가 다르다.
