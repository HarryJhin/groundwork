...
설계 문서가 나오면 저자가 같은 세션에서 한 번 self-review하고 사용자 승인을 받는다.  
...
| design-review           | 설계 저자가 문서를 한 번 self-review                             |
...
설계 리뷰에는 서브에이전트를 쓰지 않는다.  
저자가 사용자 요청·역인터뷰 답변·리포 실물과 설계 문서를 한 번에 대조한다.  
코드 리뷰만 별도 리뷰어를 쓴다.
...
- Windows 네이티브 환경은 지원하지 않는다.  
  훅이 bash 스크립트다
- 설계 리뷰는 저자의 self-review라 독립된 관측 위치를 제공하지 않는다.  
  대신 리뷰어 디스패치와 반복 라운드의 시간·토큰 비용을 없앤다
