# Standard document practice at major companies

Google, Android, Apple, Meta, AWS, Microsoft가 공개 리포에서 실제로 무엇을 두는지 관찰한 레퍼런스.  
CoC 소스, CLA 채택, SECURITY 보고 경로처럼 GitHub이 규정하지 않아 프로젝트가 정해야 하는 항목을 판단할 때 참고한다.
...
| 조직         | 중앙에서 관리하는 표준 문서            | CoC 소스                                         | CLA                              | SECURITY 보고 경로                            |
|--------------|----------------------------------------|--------------------------------------------------|----------------------------------|-----------------------------------------------|
...
| Microsoft    | CoC·SECURITY 2종                       | 자체 "MS OSS CoC" (Covenant 2.0)                 | 리포에 CLA 디렉터리 존재         | MSRC 포털 msrc.microsoft.com/create-report    |

## Analysis by axis

조사한 조직들의 실무를 네 축으로 갈라 본다.

### The de facto common denominator for a CoC is Contributor Covenant

- Apple·AWS·Meta·Microsoft는 Contributor Covenant 파생이다.  
  Apple은 표준 문안 (`gh api repos/apple/.github/contents/CODE_OF_CONDUCT.md`), Meta는 1.4 (`gh api repos/facebook/react/contents/CODE_OF_CONDUCT.md`), AWS는 2.1 (`gh api repos/aws/.github/contents/CODE_OF_CONDUCT.md` + [^aws-coc]), Microsoft는 2.0이다.  
  Microsoft는 리포 CoC 파일이 짧은 스텁이라 Covenant 언급이 없고 정책 사이트 attribution에 "adapted from the Contributor Covenant, version 2.0"으로 명시된다 ([^ms-coc]).
...
- 버전이 조직마다 갈린다.  
  ...
  2.1은 실재하는 상위 버전이다 ([^contributor-covenant-1]).  
...
- **판단**: CoC는 Contributor Covenant 채택이 사실상 표준이고 자체 브랜딩은 대규모 조직의 오버레이일 뿐 소규모엔 불필요하다.  
  ...
  실제 각 사가 연락처를 명시한다(Meta `opensource-conduct@fb.com` `gh api repos/facebook/react/contents/CODE_OF_CONDUCT.md`, MS `opencode@microsoft.com` MS OSS CoC[^ms-coc], AWS `opensource-codeofconduct@amazon.com` AWS CoC[^aws-coc]).  
  집행 의지 없는 CoC는 장식이다.
...
- 공통 원리는 공개 이슈 금지 + 사적 경로다.  
  구체 채널만 규모에 따라 다르다.
    - 이메일 직결: AWS `aws-security@amazon.com` (`gh api repos/aws/.github/contents/SECURITY.md`, HackerOne [^aws-vdp] 병기)
...
### CONTRIBUTING splits into a generic half and a per-repo half

- AWS는 범용 CONTRIBUTING.md를 따로 두고 빌드·테스트가 고유한 리포는 전용판을 쓴다 (`gh api repos/aws/.github/contents/CONTRIBUTING.md` vs `gh api repos/aws/aws-sdk-java-v2/contents/CONTRIBUTING.md`).
- Swift는 리포 CONTRIBUTING이 swift.org의 커뮤니티 문서로 위임하는 중앙화 방식이다 (`gh api repos/apple/swift/contents/CONTRIBUTING.md` + [^contributing]).
- **판단**: 행동 기대치·라이선스·리뷰 흐름처럼 프로젝트가 달라도 같은 내용은 링크로 위임하고, CONTRIBUTING에는 이 리포에서만 참인 것을 쓴다.  
  빌드 명령, 테스트 실행 방법, 디렉터리 구조가 그것이다.  
  둘을 섞으면 어느 쪽도 최신으로 유지되지 않는다.

## Android and AOSP: a case where the GitHub document model does not apply

- AOSP는 GitHub이 아니라 Gerrit(android-review.googlesource.com) 기반이다.  
  기여는 `repo`/`git` + Gerrit 리뷰 흐름이고(`repo start` → `git commit -s` → `repo upload`) GitHub PR·`.github` 정본 모델이 적용되지 않는다 ([^submit-patches]).
- 승인 권한은 Google 직원만 가지고(Code-Review+2), CLA는 여기서도 필수이며 Google 공통 CLA와 같은 URL을 쓴다 ([^contribute]).
- **함의**: 이 스킬이 다루는 배치·인식 규칙은 GitHub 호스팅 전제의 관행이다.  
  Gerrit·GitLab 등에서는 등가물(각 플랫폼의 기여 가이드·CoC 배치)로 옮겨 생각한다.  
  문서 내용의 표준 (CoC 소스, SECURITY 원칙, CHANGELOG 형식) 은 호스팅과 무관하게 그대로 쓴다.
...
1. CoC는 Contributor Covenant + 집행 연락처.
2. SECURITY는 공개 금지 + 사적 경로 + 지원 버전. 소규모는 GitHub private reporting으로 충분.
3. CLA는 법적 필요가 있을 때만, 진입장벽 비용을 알고 채택한다.  
   기본값이 아니다.
4. CONTRIBUTING에는 이 리포에서만 참인 것을 쓰고 나머지는 링크로 위임한다.

[^aws-coc]: https://aws.github.io/code-of-conduct
[^aws-vdp]: https://hackerone.com/aws_vdp
[^contribute]: https://source.android.com/docs/setup/contribute
[^contributing]: https://www.swift.org/contributing/
[^contributor-covenant-1]: https://www.contributor-covenant.org/version/2/1/
[^ms-coc]: https://opensource.microsoft.com/codeofconduct/
[^submit-patches]: https://source.android.com/docs/setup/contribute/submit-patches
