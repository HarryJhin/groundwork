...
「깨끗한 베이스라인 확인」의 보고 문안이 그 형식이고 워크트리를 만들지 않은 경로에서는 그 사실을 대신 보고한다.
이 스킬은 다음 단계로 자동으로 넘기지 않고 호출자에게 돌아간다.

**경로 기준**: 이 문서의 상대 경로는 두 시점으로 갈린다.
「기존 격리 탐지」와 「격리 워크스페이스 생성」의 경로는 시작 시점 repo 루트 기준이고 「프로젝트 셋업」 이후의 경로는 새로 만든 워크스페이스 루트 기준이다.
...
사용자가 거절하면 제자리에서 작업하고 「프로젝트 셋업」으로 간다.
...
이미 격리된 워크스페이스였으면(「기존 격리 탐지」에서 건너뜀):
```text
...
| 상황 | 행동 |
|---|---|
| 이미 연결된 워크트리 | 생성 건너뜀 (「기존 격리 탐지」) |
| 서브모듈 안 | 일반 repo로 취급 (「기존 격리 탐지」의 가드) |
...
| 변명 | 실제 |
|---|---|
| "워크트리가 아닌 게 뻔한데 확인이 필요한가" | 「기존 격리 탐지」를 돌린다. 실행 환경이 만든 격리와 서브모듈은 둘 다 눈으로는 속는다. 탐지 커맨드가 결론을 낸다. |
