...
**입력**은 현재 확인된 실패 목록이다.
테스트 출력이나 에러 로그에서 뽑고 아래 「1. 독립 영역을 식별한다」로 영역을 가른다.
...
구현 태스크에는 쓰지 않는다.
구현자를 병렬로 띄우면 충돌한다.
플랜 실행의 동시성 규율은 `groundwork:subagent-driven-development`가 기준 문서다.
...
에이전트마다 다음을 준다.
이 목록이 프롬프트 구성의 정본이고 아래 「에이전트 프롬프트 구조」는 이것을 채운 예시다.
