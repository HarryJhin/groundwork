...
서브에이전트에 격리된 컨텍스트로 작업을 위임한다. 지시와 컨텍스트를 정밀하게 구성해 그들이 초점을 잃지 않게 한다. 그들은 세션 히스토리를 상속받지 않는다. 필요한 것만 구성해 준다. 이렇게 하면 조율에 쓸 자기 컨텍스트도 지킨다.
...
**입력**은 지금 손에 있는 실패 목록이다. 테스트 출력이나 에러 로그에서 뽑고, 아래 「1. 독립 영역을 식별한다」로 영역을 가른다. 영역이 하나로 묶이면 이 스킬을 쓰지 않는다.
