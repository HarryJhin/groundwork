...
버그는 콜스택 깊은 곳에서 드러나는 일이 많다(잘못된 디렉터리에서 git init, 엉뚱한 위치에 파일 생성, 잘못된 경로로 DB 열기). 본능은 에러가 나타난 자리를 고치는 것이지만 그것은 증상 치료다.
...
직접 원인을 찾았으면 한 단계 위로 올라갈 수 있는지 본다. 올라갈 수 있으면 거슬러 올라가고, 그것이 출처인지 묻는다. 출처가 아니면 계속 올라간다. 출처면 거기서 고치고 각 층에 검증을 더한다. 그러면 버그가 불가능해진다.

**에러가 나타난 자리만 고치지 않는다.** 원 유발점까지 거슬러 올라간다.
