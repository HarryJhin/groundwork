...
`scripts/...`와 마크다운 링크는 이 스킬 디렉터리 기준이고, 그 아래 `scripts/`와 리뷰어 프롬프트 파일이 실재한다.
...
그 뒤 통합 결정은 `groundwork:finish`(머지·PR·보관을 정하고 설계 문서의 갱신일을 올리는 스킬)로 넘긴다.  
...
격리는 구현에서 비용이고 리뷰에서 효용이다.  
실행자는 설계 문서와 앞 태스크의 판단을 이미 알고 있어 그것을 버리면 부분만 복원된다.  
리뷰어가 구현 추론을 안 봤다는 사실 자체는 판정의 근거가 된다.
...
같은 말로 부르면 아래 「Handoff」의 삭제 지시가 어느 쪽을 겨누는지 갈리지 않고, 커밋된 코드가 든 워크트리를 지우는 사고가 난다.
...
**진행 기록 파일에는 파일이 아니면 남지 않는 것을 적는다.**  
보류한 Minor, 상한에서 내린 판정과 그 근거, 막힘 사유, 분해 결과다.  
어느 태스크까지 끝났는지는 todo와 `git log`가 이미 담는다.  
위 넷은 커밋에도 todo에도 들어가지 않고 컨텍스트 압축이 먼저 지운다.
...
- `<scratch>/progress.md`에 이 설계 문서의 진행 기록이 있는지 확인한다.  
  ...
  마지막 줄이 `fix 라운드`인 태스크는 루프 중간이므로 그 다음 라운드에서 재개한다.  
  첫 줄이 다른 설계 문서를 가리키는 진행 기록은 남의 진행이다.  
  그대로 두고 자기 것을 새로 연다.
- 진행 기록을 만들 때 첫 줄에 정체를 적는다: `# 진행 기록 — 설계: <설계 문서 파일 경로>`.  
  아래 「Break the work into tasks」에서 확정한 태스크 목록을 그 다음에 적는다.  
...
## Read and review the design document

설계 문서 전문을 읽는다.  
범위·제외 범위·수용 기준·전역 제약을 파악한다.  
이 네 절의 정의는 `groundwork:finding-unknowns`의 「What the design document contains」이 정본이다.
...
갈리지 않은 채 넘어왔으면 서브시스템마다 별도 설계 문서로 쪼갤 것을 제안한다.

## Break the work into tasks

분해는 커밋되는 문서가 아니라 todo와 진행 기록으로 남는다.  
사용자 승인을 거치지 않으므로 착수 전에 결과를 대화에서 한 번 짚는다(아래 「Announce the breakdown」).

### Draw the file structure first
...
### Signs the breakdown is unfinished
...
- 리뷰어에게 diff를 파일로 넘긴다.  
  ...
  리뷰어는 커밋 목록·변경 파일 요약·문맥 포함 전체 diff를 한 번의 Read로 본다.  
...
- **리뷰어마다 모델을 명시해 띄운다.**  
  티어는 [choosing-model-tier.md](../using-groundwork/choosing-model-tier.md)의 「Dispatch axis: code review tasks」에 있다.  
...
- Minor 발견은 진행하며 진행 기록에 적는다(`태스크 <N>: minor(보류): <한 줄>`).  
  「Final review」가 그 목록을 보고 머지 전에 무엇을 고쳐야 할지 분류한다.  
  Minor는 루프에 들어가지 않는다.
...
판정은 3라운드 상한에서만 하고 모든 판정은 진행 기록 항목이다.  
조용한 폐기는 금지다.
...
남은 발견은 「Fix loop」의 상한 판정과 같은 방식으로 판정한다.  
...
진행 기록이 사라지면 「Fix loop」와 「Final review」가 남긴 보류 판정과 막힘 사유가 사용자에게 도달할 경로가 없다.
...
- 검증이 반복해서 실패한다
- 설계 문서가 요구한 것이 구현에서 성립하지 않는 것으로 드러났다
...
마지막 항목은 회귀가 아니라 개정으로 간다.  
처리는 아래 「Revise the design document」이 정한다.

## Revise the design document

구현이 설계 문서와 갈라지면 설계 문서를 고친다.  
설계 문서는 닫히지 않으므로 언제든 고칠 수 있다.  
갈라진 사실을 진행 기록에만 남기면 그 스크래치는 종료 시 사라지고 설계 문서는 틀린 내용으로 굳어버린다.  
문서를 고쳤으면 frontmatter의 `updated`를 그날 날짜로 맞춘다.

**아직 출시하지 않았으면 무조건 갱신한다.**  
반영하지 않으면 나중에 이 시스템을 맡는 사람이 만들어진 것이 아니라 만들려던 것을 읽는다.

**갱신 대상은 설계이지 구현 실수가 아니다.**  
설계가 요구한 것을 구현이 잘못 따라간 것이면 구현을 고친다.  
설계의 결정·가정·요구·트레이드오프가 실제로 바뀌었으면 문서를 고친다.  
둘을 가르는 물음은 "지금 이 설계를 처음부터 다시 쓴다면 같은 결정을 적겠는가"다.  
같은 결정을 적겠으면 구현이 틀린 것이고, 다르게 적겠으면 설계가 바뀐 것이다.  
구현이 어렵다는 이유로 문서를 고치면 갈라진 사실을 지우는 것이지 해소하는 것이 아니다.
...
**뒷줄은 재리뷰만 돈다.**  
고친 절을 담당하는 렌즈에만 `groundwork:design-review`를 돌리고 로스터 전원을 부르지 않는다.  
그 재리뷰 규칙은 그 스킬의 「Review rules」에 있다.  
반환을 받은 뒤 무엇을 왜 고쳤는지 사용자에게 통지하고 답을 기다리지 않는다.

어느 줄이든 문서를 고친 뒤 그 태스크를 개정된 문서로 다시 시작한다.  
이미 만든 것을 그대로 둔 채 문서만 맞추면 다음 태스크의 리뷰어가 개정 전 구현을 개정 후 계약으로 판정한다.

## Things to remember

- 설계 문서를 먼저 비판적으로 검토한다
- 설계 문서에 없던 판단이 나오면 보수적으로 택해 진행하고 기록한다.  
  뒤에 오는 작업이 그 판단에 기대게 되면 설계 문서 「Design」에 그 결정을 더한다(위 「Revise the design document」).  
...
| "설계 문서가 계약이니 태스크는 알아서"      | 「Signs the breakdown is unfinished」에 걸리는 것이 하나라도 있으면 착수 전에 없앤다.                                                                   |
| "끝났으니 스크래치를 치우고 넘긴다"         | 진행 기록이 `groundwork:finish`의 「Report the outstanding findings」 입력이다. 지우면 보류 판정이 사용자에게 도달하지 않는다. 정리는 그 스킬의 몫이다. |
