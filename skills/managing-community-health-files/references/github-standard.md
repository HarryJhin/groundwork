...
GitHub 공식 문서 기준의 배치 규칙, 상속 모델, 파일별 문법 레퍼런스. 스캐폴딩하거나
...
- `.github`라는 이름의 **public** 리포에 헬스 파일을 두면, 같은 계정(조직 또는 개인)
...
- 이슈 템플릿이 label을 지정하면, 그 label은 `.github` 리포와 템플릿을 쓰는 모든
  리포에 존재해야 한다.
- **LICENSE는 default로 만들 수 없다.** clone·package·download에 포함되도록 개별
  리포에 직접 둔다.

### 지원되는 default 파일 목록

CODE_OF_CONDUCT.md, CONTRIBUTING.md, Discussion category forms(`DISCUSSION_TEMPLATE/`),
FUNDING.yml, GOVERNANCE.md, 이슈·PR 템플릿과 config.yml, SECURITY.md, SUPPORT.md.
...
- community profile 체크리스트에서 "Code of conduct"가 완료로 표시되는 것은
...
`.github/FUNDING.yml`(default 브랜치)에 둔다. 플랫폼당 항목 1개, custom URL은 최대 4개.
...
위가 현재 문서에 실린 플랫폼 전부다. 목록은 GitHub이 늘리므로 낯선 플랫폼을 요청받으면
출처 페이지를 다시 확인한다.
...
출처: https://docs.github.com/en/communities/using-templates-to-encourage-useful-issues-and-pull-requests/about-issue-and-pull-request-templates
...
- 이슈 템플릿은 default 브랜치의 `.github/ISSUE_TEMPLATE`에만 둔다. 마크다운은 `.md`,
  이슈 폼은 `.yml`.
- community profile 체크마크 조건: `.github/ISSUE_TEMPLATE`에 위치 + 마크다운은
  frontmatter에 유효한 `name:`·`about:`, 이슈 폼은 유효한 `name:`·`description:` 키.
- 템플릿 이름은 3자를 초과해야 이슈 생성 화면에 보인다.
- 파일은 알파벳순 정렬되고 타입별로 그룹핑된다(YAML이 Markdown보다 먼저). 파일명 숫자
  프리픽스(`1-bug.yml`, 10개 이상은 `01-`)로 순서를 제어한다.
- 다중 PR 템플릿은 지원 폴더 안 `PULL_REQUEST_TEMPLATE/` 하위 디렉터리에 두고
  `?template=` 쿼리 파라미터로 선택한다.
...
body 요소 스키마: https://docs.github.com/en/communities/using-templates-to-encourage-useful-issues-and-pull-requests/syntax-for-githubs-form-schema
...
- code owner는 리포에 명시적 **write 권한**이 필요하다. 팀 형식은 `@org/team-name`.
- **마지막으로 매칭되는 패턴이 최우선**이다(순서가 중요). 한 패턴에 여러 owner를
  지정하려면 모두 **같은 줄**에 둔다. 다른 줄이면 마지막 것만 매칭된다.
- 경로는 대소문자를 구분한다. 유효하지 않은 라인은 스킵되고 REST API로 확인된다
  (`gh api repos/{owner}/{repo}/codeowners/errors`).
- gitignore 패턴을 대부분 따르나 CODEOWNERS에서 **안 되는 것**: `#`을 `\`로 이스케이프,
  `!` negation, `[ ]` 문자 범위.
...
- `[권장]` 리포 완전 보호를 위해 CODEOWNERS 파일 자체의 owner도 정의한다. 가장 안전한 방법은
  `.github/` 디렉터리에 CODEOWNERS를 두고 `/.github/ @owner`로 그 디렉터리를 커버하는 것이다.

## community profile

출처: https://docs.github.com/en/communities/setting-up-your-project-for-healthy-contributions/about-community-profiles-for-public-repositories
(REST API는 https://docs.github.com/en/rest/metrics/community)
...
- 응답의 `files` 객체(code_of_conduct, code_of_conduct_file, contributing, issue_template,
  ...
  주는 필드일 뿐이다. 이 둘을 같은 것으로 읽으면 안 된다.

### 산식 관측 (문서 미규정, 재현 가능)

아래는 공식 사실이 아니라 리포 18개를 조회해 역산한 관측이다. GitHub이 예고 없이 바꿀 수 있다.
