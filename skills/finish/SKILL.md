...
**입력**은 방금 끝낸 작업의 워크스페이스다.
커맨드는 그 워크스페이스 안에서 시작한다.
「산출물 종료 표기」가 종료 표기할 설계 문서 파일의 경로는 호출자에게 받는다.
`groundwork:executing-design`이나 `groundwork:subagent-driven-development`가 이 스킬을 부를 때 실행한 설계 문서 경로를 함께 넘긴다.
경로를 받지 못했으면 사용자에게 묻고 사용자가 산출물이 없다고 하면 「산출물 종료 표기」를 건너뛴다.

**주체**는 작업을 마친 에이전트다.
아래에서 무주어로 적은 지시는 전부 그 에이전트 몫이고 "사용자"는 통합을 결정하는 사람이다.

**출력**은 사용자가 고른 통합 방식의 실행 결과다.
선택과 그 결과, 워크트리·브랜치를 어떻게 처리했는지, 산출물에 종료 표기를 남겼는지를 보고하고 끝낸다.
...
# 「선택 실행」이 디렉터리를 옮기고 「워크스페이스 정리」가 이 값을 필요로 한다
WORKTREE_PATH=$(git rev-parse --show-toplevel)
FEATURE_BRANCH=$(git branch --show-current)   # detached HEAD면 빈 문자열
```

**「테스트 확인」부터 「워크스페이스 정리」까지의 셸 블록은 같은 셸에서 돈다.**
...
두 메뉴의 문안은 「선택지 제시」에 있다.

| 상태 | 메뉴 | 정리 |
|---|---|---|
| `GIT_DIR == GIT_COMMON` (일반 repo) | 표준 3선택 (「선택지 제시」) | 정리할 워크트리 없음 |
| `GIT_DIR != GIT_COMMON`, 이름 있는 브랜치 | 표준 3선택 (「선택지 제시」) | 출처 기반 (「워크스페이스 정리」) |
| `GIT_DIR != GIT_COMMON`, detached HEAD | 축소 2선택 (「선택지 제시」, 머지 없음) | 외부 관리. 그대로 둔다 |
...
작업 폐기는 사용자가 명시로 요청할 때만 일어난다(아래 「사용자의 폐기 요청」).
답을 기다린다.
통합 결정은 사용자의 것이다.

## 선택 실행

「선택지 제시」에서 사용자가 고른 번호에 해당하는 절만 실행한다.
...
`BASE_BRANCH`는 「base 브랜치 확인」에서 확인한 base 브랜치 이름이다.
...
머지 결과가 green이면 워크트리를 정리하고(「워크스페이스 정리」) 브랜치를 지운다.
...
그다음 워크트리를 정리하고(「워크스페이스 정리」) 브랜치를 강제 삭제한다.
...
그 이동 전에 「환경 탐지」에서 잡아둔 `GIT_DIR`·`GIT_COMMON`·`WORKTREE_PATH` 값을 쓴다.
...
종료 표기 대상은 설계 문서 하나다.
태스크 분해는 실행 스킬의 진행 기록에만 있었고 그 워크스페이스는 실행 스킬이 이미 지웠다.
