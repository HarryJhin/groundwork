...
7단계가 종료 표기할 스펙·플랜 파일의 경로는 호출자에게 받는다.
`groundwork:executing-plan`이나 `groundwork:subagent-driven-development`가 이 스킬을 부를 때 실행한 플랜 경로와 그 플랜 Goal 절이 지목한 스펙 경로를 함께 넘긴다.
경로를 받지 못했으면 사용자에게 묻고 사용자가 산출물이 없다고 하면 7단계를 건너뛴다.
...
작업 폐기는 사용자가 명시로 요청할 때만 일어난다(아래 「사용자가 폐기를 요청하면」).
답을 기다린다.
통합 결정은 사용자의 것이다.
...
`BASE_BRANCH`는 3단계에서 확인한 base 브랜치 이름이다.
...
그 이동 전에 2단계에서 잡아둔 `GIT_DIR`·`GIT_COMMON`·`WORKTREE_PATH` 값을 쓴다.
