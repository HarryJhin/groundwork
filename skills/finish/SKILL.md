---
name: finish
description: 구현이 끝나고 테스트가 통과한 뒤 작업을 통합하는 방법을 정하고 설계 문서의 갱신일을 올린다. Use when 전 태스크 완료, 브랜치 마무리, 머지·PR 결정, 개발 종료.
---
...
선택과 그 결과, 워크트리·브랜치를 어떻게 처리했는지, 설계 문서의 갱신일을 올려 커밋했는지, 실행 스크래치를 지웠는지 남겼는지를 보고하고 끝낸다.

**절 순서가 곧 실행 순서다.**  
「Confirm the tests」에서 「Clean up the execution scratch」까지 위에서 아래로 돈다.  
순서를 바꾸면 아래 셋이 깨진다.  
갱신일 기입은 통합보다 앞서야 그 커밋이 통합에 실린다.  
...
# 「Execute the choice」이 디렉터리를 옮기고 「Clean up the worktree」에 이 값이 필요하다
...
**이것은 문항이 아니다.**  
답을 요구하지 않고 곧바로 「Stamp the updated date」로 간다.  
...
실행한 설계 문서의 `updated`를 오늘 날짜로 맞춘다.  
대상은 설계 문서 하나다.  
태스크 분해는 실행 스크래치의 진행 기록에만 있었고 그것은 커밋 산출물이 아니다.

기입 형식은 이렇다.  
frontmatter 블록 내부의 `^updated:` 라인만 교체한다(중복 삽입 금지).  
라인이 없으면 `created` 다음 줄에 삽입한다.  
frontmatter 자체가 없으면 파일 최상단에 4행(`---` / `created: <생성일>` / `updated: <오늘>` / `---`)을 신설한다.  
두 라인 모두 첫 10행 안에 둔다.

**설계 문서는 닫히지 않는다.**  
이 값은 종료 표기가 아니라 마지막으로 손댄 날이다.  
나중에 이 설계를 바꾸면 그 문서를 다시 고치고 `updated`를 다시 맞춘다.  
그래서 이 스킬을 돈 뒤에도 그 문서는 여전히 가변이다.

**기입하고 커밋한다.**  
설계 문서는 커밋 산출물이라 커밋되지 않은 갱신은 리포에 남지 않는다.

```bash
git add DESIGN_FILE
git commit -m "설계 문서의 갱신일을 올린다"
```
...
| 선택               | 갱신일이 남나 | 머지 | 푸시 | 워크트리 유지 | 실행 스크래치 유지 | 브랜치 정리 |
|--------------------|---------------|------|------|---------------|--------------------|-------------|
| 1. 로컬 머지       | ○             | ○    | ×    | ×             | ×                  | ○           |
| 2. PR 생성         | PR 머지 시    | ×    | ○    | ○             | ○                  | ×           |
| 3. 그대로 둠       | 브랜치에      | ×    | ×    | ○             | ○                  | ×           |
...
갱신일 커밋은 어느 선택에서든 브랜치에 남는다.  
그것이 base에 남는지는 통합 방식이 정한다.  
폐기는 브랜치를 강제 삭제하면서 그 커밋도 함께 버리므로 갱신이 남지 않는다.  
버린 작업이 기록되지 않는다는 뜻이라 이것이 옳은 결과다.  
...
| "갱신일은 마무리니까 맨 끝에 올린다"             | 맨 끝은 머지·푸시 뒤다. 그때 기입한 값은 실을 커밋이 없어 리포에 남지 않는다. 통합보다 먼저 기입하고 커밋한다. |
| "날짜 한 줄 고치는 건 커밋할 거리가 아니다"      | 설계 문서는 커밋 산출물이다. 커밋하지 않은 갱신은 작업 트리에만 있다가 사라진다.                               |
| "이 스킬을 돌았으니 설계 문서는 닫혔다"          | 닫는 표기는 없다. `updated`는 마지막으로 손댄 날이고 그 뒤에도 문서는 고칠 수 있다.                            |
