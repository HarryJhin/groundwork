...
**출력**: 전 태스크를 마치면 무엇을 구현했고 어느 검증이 통과했는지 사용자에게 보고하고 `groundwork:finish`(머지·PR·보관을 정하는 스킬)로 넘긴다.
...
다만 그 스킬도 리뷰어는 서브에이전트로 띄우므로, 서브에이전트가 아예 없으면 리뷰 게이트를 벤더 교차 디스패치로 대체한다([choosing-model-tier.md](../using-groundwork/choosing-model-tier.md)의 「벤더 교차 디스패치」).
...
- 진행 기록을 만들 때 첫 줄에 정체를 적는다: `# SDD 진행 기록 — 설계: <설계 문서 파일 경로>`.
  아래 「분해」에서 확정한 태스크 목록을 그 다음에 적는다.
...
분해의 규범은 `groundwork:executing-design`의 「태스크로 분해한다」가 정본이다.
파일 구조를 먼저 그리고, 태스크 경계를 빌드·테스트 통과 지점에 두고, 태스크마다 건드리는 파일·인터페이스·검증 커맨드·선행 태스크를 확정하고, 「분해가 미완이라는 신호」에 걸리는 것을 착수 전에 없앤다.
...
- 서로 모순되거나 설계 문서의 「전역 제약」과 모순되는 요구
...
- **태스크 브리프**: 분해에서 확정한 이 태스크의 내용을 워크스페이스에 파일로 쓴다(`<workspace>/task-<N>-brief.md`).
...
- 리뷰어에게 넘기는 전역 제약 블록이 그 리뷰어의 주의 렌즈다.
  설계 문서의 「전역 제약」과 「수용 기준」에서 구속력 있는 요구를 그대로 복사한다.
...
**4~5라운드. 더 유능한 모델로 fresh 구현자를 디스패치한다**(위 「모델 선택」).
...
가장 유능한 모델로 디스패치하고(위 「모델 선택」) `groundwork:requesting-code-review`의 [code-reviewer-prompt.md](../requesting-code-review/code-reviewer-prompt.md)를 쓴다.
...
| 변명 | 실제 |
|---|---|
| "서브에이전트가 기본이니 일단 이걸로" | 기본은 `groundwork:executing-design`이다. 「적용 판단」의 세 조건을 전부 만족할 때만 이 경로다. |
