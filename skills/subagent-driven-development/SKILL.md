...
입력은 승인된 설계 문서 경로다. 메인 컨트롤러가 분해·조정을 맡고 태스크별 구현자와 리뷰어를 격리한다. 실행 대상 식별과 경로 기준은 `groundwork:executing-design`의 「Execution contract」를 따른다.
...
설계 읽기·스크래치 확보·진행 기록·재개·태스크 분해는 `groundwork:executing-design`의 「Set up and resume」와 「Break the work into tasks」를 따른다.
...
태스크 시작·끝 커밋을 기록하고 `groundwork:executing-design`의 「Review the task」에 따라 패키지 경로·브리프·리포트·정확한 제약을 격리된 리뷰어에게 넘긴다.
발견의 결정 권한은 `groundwork:executing-design`의 「Resolve findings」를 따른다.
...
최종 리뷰와 설계 개정은 `groundwork:executing-design`의 「Final review and handoff」와 「Revise the design document」를 따른다. 스크래치나 워크트리를 먼저 지우지 않는다.
