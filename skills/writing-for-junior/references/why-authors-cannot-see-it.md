출처:
- [^curseknowledgeeconset-pdf]
- [^s-1467-9280-00439]

# Why authors cannot see the context missing from their own documents

두 편이 같은 현상의 다른 층을 다룬다.  
아는 사람이 모르는 사람의 상태를 재구성하지 못한다는 것(curse of knowledge)과, 화자가 자기 의도의 전달 성공률을 실제보다 높게 잡는다는 것(illusion of transparency)이다.

## Camerer, Loewenstein & Weber (1989)

"The Curse of Knowledge in Economic Settings: An Experimental Analysis". Journal of Political Economy 97(5), 1232–1254.

경제학에서 "더 많은 정보를 가진 행위자가 덜 가진 행위자의 판단을 정확히 재현할 수 있다"는 가정이 체계적으로 깨진다는 것을 실험으로 보인다.  
더 잘 아는 쪽은 사적 정보를 무시하는 것이 자기 이익일 때조차 무시하지 못한다.  
저자들이 이 현상에 curse of knowledge라는 이름을 붙였다.

시장 실험과 개인 판단 실험을 비교해 시장 압력이 이 편향을 대략 절반으로 줄이지만 없애지는 못한다는 결과를 얻는다.

## Keysar & Henly (2002)

"Speakers' Overestimation of Their Effectiveness". Psychological Science 13(3).

화자가 중의적 문장으로 특정 의도를 전달하게 하고 청자가 그 의도를 이해했을지 화자에게 추정시킨다.  
화자는 자기가 의도를 성공적으로 전달했다고 믿은 비율이 72%였고 청자가 실제로 그 의미를 이해한 비율은 61%였다.  
저자들은 이 과대평가를 illusion of transparency로 명명한다.

제시된 설명은 둘이다.  
화자에게는 자기 의도가 투명하지만 청자의 판단은 실제로 전달된 것에만 묶인다는 것, 그리고 발화 행위 자체가 화자의 인지 자원을 소모해 청자를 관찰할 여력을 남기지 않는다는 것이다.  
둘이 겹쳐 화자가 자기중심적 기준점에 묶인다.

## Where the skill uses this

[Review readability](../SKILL.md#review-readability)의 격리된 독자 검토를 뒷받침하는 근거다.

- 저자가 작성 대화를 안다는 이유로 독자의 맥락을 가정하지 않게 하는 판단에 curse of knowledge를 적용했다.
- 판정 축을 저자 자신이 아니라 격리된 리뷰어에게 맡기는 설계가 여기서 나온다.  
  저자에게 "맥락 없이 읽어보라"고 요구하는 것은 이 편향이 요구를 듣지 않는다는 실험 결과와 충돌한다.
- illusion of transparency의 72% 대 61% 간극은 리뷰 없이 저자 자기점검만 두면 남는 잔차의 크기를 가늠하게 한다.
- 판독 축 정의의 "네가 아는 것은 근거가 아니다" 조항은 리뷰어에게도 같은 편향이 걸린다는 전제에서 나왔다.  
  리뷰어가 도메인 지식을 지닌 채 판정하면 저자와 같은 처지가 된다.

## Verification scope

Camerer 등의 서지 정보와 핵심 주장, 시장이 편향을 절반가량 줄인다는 결과는 검색 결과 요약과 서지 페이지에서 확인했고 논문 전문은 대조하지 않았다.  
Keysar & Henly의 72%/61% 수치와 illusion of transparency 명명, 두 가지 설명은 검색 결과 요약에서 확인했고 원논문 전문은 대조하지 않았다.

두 연구 모두 문서 작성이 아니라 각각 경제적 판단과 구어 대화를 다룬다.  
문서 저자와 문서 독자 사이로 옮기는 것은 이 스킬의 유추이고 두 논문의 주장이 아니다.  
인용을 다른 문서로 옮길 때 이 표기를 함께 옮긴다.

흔히 함께 인용되는 Newton(1990)의 tapping study는 이번 조사에서 원출처를 확인하지 않았으므로 근거로 쓰지 않았다.

[^curseknowledgeeconset-pdf]: https://www.cmu.edu/dietrich/sds/docs/loewenstein/CurseknowledgeEconSet.pdf
[^s-1467-9280-00439]: https://journals.sagepub.com/doi/abs/10.1111/1467-9280.00439
