...
오픈소스 프로젝트에 처음 들어오는 사람이 부딪히는 장벽을 문헌 리뷰로 모아 다섯 범주의 계층 모델로 정리한다. 이 스킬이 "주니어"를 능력이 아니라 맥락의 부재로 정의한 근거다.

## 다섯 범주와 장벽 (Table 1 verbatim)
...
- **독자 정의의 근거**다. 이 모델은 기술 경험(Previous Technical Experience)과 도메인 지식(Domain expertise)을 별도 장벽으로 분리한다. 이 스킬이 독자를 "기술은 알지만 이 프로젝트는 모르는 사람"으로 정의하고 "주니어는 능력이 아니라 맥락의 부재"라고 못박은 것이 이 분리에 대응한다.
